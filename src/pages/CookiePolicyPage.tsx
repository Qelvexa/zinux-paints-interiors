...
export const CookiePolicyPage: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/api/legal?slug=cookie')
      .then(res => res.json())
      .then(data => {
        if (data) {
          setTitle(data.title || 'Cookie Policy');
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-5 sm:p-8 lg:p-12 shadow-sm space-y-5 sm:space-y-6 text-slate-700 text-xs sm:text-sm leading-relaxed">
          ...
          {loading ? (
            ...
            <div className="whitespace-pre-wrap leading-relaxed space-y-4">
              {content || (
                <p>
                  This Cookie Policy explains how {settings.business_name} utilizes cookies and related technologies for security, preference storage, and quote request handling.
                </p>
              )}
            </div>
          )}
